class PurchaseReceival < ActiveRecord::Base
  ...
  def delete_object
    ...
    if self.purchase_receival_entries.count != 0
      self.errors.add(:generic_errors, "Sudah ada purchase receival entry")
      return self 
    end
    
    self.purchase_receival_entries.each {|x| x.delete_object}
    self.destroy
  end
  ...
  def unconfirm 
    ...
    self.save
    self.purchase_receival_entries.each {|x| x.unconfirm }
  end
end
